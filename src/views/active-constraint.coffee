...
class exports.ActiveConstraint extends Backbone.View

    tagName: "form"
    className: "form-inline im-constraint row-fluid"
    
    BASIC_OPS = Query.ATTRIBUTE_VALUE_OPS.concat Query.NULL_OPS
    ...
    initialize: (@query, @orig) ->
        ...
        @type = @path.getType()
        @cast = if @type in Model.NUMERIC_TYPES then ((x) -> 1 * x) else ((x) -> '' + x)
        @con = new Backbone.Model(_.extend({}, @orig))
        if @path.isReference() or @path.isRoot()
            @ops = Query.REFERENCE_OPS
        else if @path.getType() in Model.BOOLEAN_TYPES
            @ops = ["=", "!="].concat(Query.NULL_OPS)
        else if @con.has('values')
            @ops = Query.ATTRIBUTE_OPS
        else
            @ops = BASIC_OPS
    ...
    setError: (key) ->
      @$el.addClass 'error'
      @$('.im-conbuilder-error').text getMessage "conbuilder.#{ key }"
      false
    ...
    editConstraint: (e) ->
        ...
        if @con.get('op') in Query.MULTIVALUE_OPS.concat(Query.NULL_OPS)
            @con.unset('value')
        if @con.get('op') in BASIC_OPS
            @con.unset('values')

        if (@con.get('op') in Query.MULTIVALUE_OPS) and @con.get('values').length is 0
    ...
    addIcons: ($label) ->
        ...
        if @con.locked
            ...
            $label.append """<a><i class="im-edit #{ icons.Edit }"></i></a>"""
    ...
    getTitleOp: () -> @con.get('op') or getMessage 'conbuilder.IsA'
    getTitleVal: () ->
        if @con.get('values')
    ...
    fillConSummaryLabel: () ->
      ...
      unless @con.get('op') in Query.NULL_OPS
          val = @getTitleVal()
          ul.append toLabel val, 'value' if val?
          if @con.has 'extraValue'
              ul.append getMessage('conbuilder.ExtraLabel')
    ...
    drawTypeOpts: (fs) ->
      ...
      for t in types then do (t) ->
        ...
        schema.getPathInfo(t).getDisplayName().then (name) ->
          option.text name
    ...
    TOO_MANY_SUGGESTIONS = _.template """
      <span class="alert alert-info">
        ...
        #{ getMessage('conbuilder.TooManySuggestions') }
    ...
    drawAttributeOpts: (fs) ->
        input = $ """
            <input class="span7 im-constraint-value im-value-options im-con-value" type="text"
                placeholder="#{ getMessage('conbuilder.ValuePlaceholder') }"
    ...
    provideSuggestions: (input) ->
      ...
      filtering = clone.filterSummary pstr, "", options.MaxSuggestions
    ...
    drawExtraOpts: (fs) ->
        fs.append """
            <label class="im-value-options">
                #{ getMessage('conbuilder.ExtraLabel') }
                <input type="text" class="im-extra-value"
                    placeholder="#{ getMessage('conbuilder.ExtraPlaceholder') }"
    ...
    drawValueOptions: ->
        ...
        if not currentOp and @con.has('type')
            @drawTypeOpts(fs)
        else if (@path.getType() in Model.BOOLEAN_TYPES) and not (currentOp in Query.NULL_OPS)
            @drawBooleanOpts(fs)
        else if currentOp in Query.MULTIVALUE_OPS
            @drawMultiValueOps(fs)
        else if currentOp in Query.LIST_OPS
            @drawListOptions(fs)
        else if @path.isReference() and (currentOp in ['=', '!='])
            @drawLoopOpts(fs)
        else if not (currentOp in Query.NULL_OPS)
            @drawAttributeOpts(fs)
